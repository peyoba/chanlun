...
- 笔中枢不跨线段（M2 起，cfg.bi_zs_cross_seg=False）：元素序列按线段切分后分别构造。
"""
...
def _build(elems: Sequence, kind: str, cfg: ZsConfig, offset_hint: str = "") -> list[ZhongShu]:
    out: list[ZhongShu] = []
    n = len(elems)
    i = 1  # 首元素至少要有进入段
    while i + 2 < n:
...
def build_bi_zs(bis: list[Bi], segs: list[Seg], cfg: ZsConfig, merged: list[MergedBar] | None = None) -> list[ZhongShu]:
    ...
    out: list[ZhongShu] = []
    for s in segs:
        # 线段内部笔序列，前一段的最后一笔作为进入段参考
        lo = max(0, s.start_bi - 1)
        sub = _build(bis[lo:s.end_bi + 1], "bi", cfg)
        for z in sub:
